...
let currentState = {
    ...
    selectedBlockType: 'heading', // Default to Heading
    inputText: ""
};
        ...
        <div id="landing-page-content" class="centered-content-container">
            <div class="landing-page-vstack">
                ...
                <div class="input-content-box">
                    <textarea id="main-input-field" class="search-input" placeholder="${getPlaceholder()}" rows="1" spellcheck="true" autocomplete="on" autocorrect="on" autocapitalize="sentences">${currentState.inputText}</textarea>
                    <div class="control-row">
                        <select id="mode-select" class="control-pill mode-selector">
                            ${Object.values(InputMode).filter(mode => mode !== InputMode.WORKSPACES).map(mode => `<option value="${mode}" ${currentState.selectedMode === mode ? 'selected' : ''}>${mode}</option>`).join('')}
                        </select>
...
function renderDynamicSelector() {
    if (currentState.selectedMode === InputMode.SEARCH) {
    ...
    } else if (currentState.selectedMode === InputMode.NOTE) {
        const blockTypes = [
        ...
        return `<select id="search-model-select" class="control-pill">${blockTypes.map(t => `<option value="${t.id}" ${currentState.selectedBlockType === t.id ? 'selected' : ''}>${t.name}</option>`).join('')}</select>`;
    }
    return '';
}
...
function attachLandingPageListeners() {
    ...
    const dotWorkspaces = document.getElementById('dot-workspaces');
    ...
    const triggerAction = () => {
        ...
        window.handlePerformAction({
            ...
            blockType: currentState.selectedBlockType // Pass block type
        });
    ...
    const attachDynamicListener = () => {
        ...
        if (dynamicSelect) {
            dynamicSelect.addEventListener('change', (e) => {
                ...
                else if (currentState.selectedMode === InputMode.LLM) currentState.selectedModel = e.target.value;
                else if (currentState.selectedMode === InputMode.NOTE) currentState.selectedBlockType = e.target.value;
            });
        }
    };
    ...
    if (inputField) {
        inputField.addEventListener('contextmenu', (e) => {
            console.log('[Renderer] Context Menu Event on Input:', e);
        });
    }
