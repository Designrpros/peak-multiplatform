...
const { renderCommandResultCard } = require('./sub-cards/CommandResultCard');
const { renderTerminalCard } = require('./sub-cards/TerminalCard');
// const ExecuteCard = require('./cards/ExecuteCard'); // Legacy
const UnifiedTaskCard = require('./cards/UnifiedTaskCard');
const ClarificationCard = require('./cards/ClarificationCard');
const PlanReviewCard = require('./cards/PlanReviewCard');
...
class ChatView {
    ...
    attachListeners() {
        ...
        if (docsBtn && docsMenu) {
            ...
            this.container.addEventListener('click', async (e) => {
                ...
                console.log('[ChatView] Button clicked:', btn.className, btn.dataset);

                // Handle tool execution buttons
                if (btn.classList.contains('tool-run-btn') ||
                    btn.classList.contains('tool-create-btn') ||
                    btn.classList.contains('file-action-btn-compact') ||
                    btn.dataset.action === 'execute') {

                    console.log('[ChatView] Tool execution button detected');
                    e.stopPropagation();
        ...
        window.addEventListener('peak-workflow-state-change', (e) => {
            const { state, agentId } = e.detail;
            console.log('[ChatView] Workflow state changed:', state);

            if (state === 'EXECUTING') {
                // UnifiedTaskCard handles execution phase internally via stream updates
                // We do NOT create a new card here to avoid breaking the unified flow
                console.log('[ChatView] Entering EXECUTION phase (handled by UnifiedTaskCard)');

                // Optional: If we wanted to force a phase marker, we could do it here
                // if (this.currentTaskCard && this.currentTaskCard.addStep) {
                //     this.currentTaskCard.addStep('phase', [], 'Execute');
                // }
            }
        });

        // Listen for Plan Approval
        window.addEventListener('peak-plan-approve', (e) => {
            console.log('[ChatView] Plan approved');
            const AgentOrchestrator = require('../core/AgentOrchestrator');
            AgentOrchestrator.handlePlanApproval(true);
        });

        // Listen for Plan Rejection
        window.addEventListener('peak-plan-reject', (e) => {
            console.log('[ChatView] Plan rejected');
            const AgentOrchestrator = require('../core/AgentOrchestrator');
            AgentOrchestrator.handlePlanApproval(false, e.detail.feedback);
        });
        ...
        window.addEventListener('mcp:stream-complete', (e) => {
            if (this.currentTaskCard) {
                ...
                // Stop all spinners in code-thinking blocks
                const spinners = this.currentTaskCard.element.querySelectorAll('.code-thinking-spinner');
                spinners.forEach(spinner => {
                    spinner.style.animation = 'none';
                    spinner.style.opacity = '0.5';
                });

                // IMPORTANT: Wait for DOM to update before checking for buttons
                // The final tool cards might not be in the DOM yet when this event fires
                setTimeout(() => {
        ...
        // Event delegation for code toggle buttons
        this.chatThread.addEventListener('click', (e) => {
            const toggleBtn = e.target.closest('.toggle-code-btn-compact');
            if (toggleBtn) {
                const card = toggleBtn.closest('.file-edit-card-compact');
                if (card) {
                    const codeDiv = card.querySelector('.file-code-collapsed');
                    if (codeDiv) {
    ...
    renderHistory(forceClear = false) {
        ...
        while (i < history.length) {
            ...
            if (msg.role === 'user') {
                ...
                const UnifiedTaskCard = require('./cards/UnifiedTaskCard');
                const card = new UnifiedTaskCard(userPrompt, msg.agent || 'general', msg.commitHash);
    ...
    renderSettings() {
        ...
        const generalSettings = [
            ...
            { id: 'peak-auto-accept-plan', label: 'Auto-accept Plan Files (implementation_plan.md, TODO.md)', default: false },
        ];

        generalSettings.forEach(setting => {
            console.log('[ChatView] Rendering setting:', setting.id, setting.label);
            const item = document.createElement('div');
    ...
    async handleTerminalResponse(e) {
        ...
        // Auto-continue: Send tool result to AI automatically
        this.continueWithSystemMessage(`Command Execution Result:\n${outputContent}`);
    ...
    /**
     * Continues the AI conversation by sending a system message (e.g., tool result)
     * This replaces the old "continue" command approach which created phantom user messages
     * @param {string} systemMessage - The system message content (e.g., tool execution result)
     */
    async continueWithSystemMessage(systemMessage) {
        console.log('[ChatView] continueWithSystemMessage called', systemMessage.substring(0, 100));

        // Prevent continuation if already streaming
        if (this.client.isStreaming) {
            console.log('[ChatView] Already streaming, skipping continuation');
            return;
        }
        ...
        try {
            ...
            const projectRoot = window.currentProjectRoot || '/project';
            ...
            // Set loading state
            this.inputBar.updateStatus('thinking', 'Processing...');

            // Send message with null prompt to trigger continuation based on history
            // The AI will see the system message and continue from there
            await this.client.sendMessage(null, context, modelId, null, systemPrompt, agent);

        } catch (err) {
            console.error('[ChatView] Error in continueWithSystemMessage:', err);
            this.inputBar.updateStatus('ready');
            this.appendMessage('system', `Error continuing conversation: ${err.message}`);
        }
    }

    /**
     * Show interactive plan review card when AI asks for plan approval
     * @param {Array} files - Array of { path, content } objects
     */
    async showPlanReviewCard(files) {
        console.log('[ChatView] showPlanReviewCard called with', files.length, 'files');

        const card = new PlanReviewCard({
            files: files,
            onApprove: () => {
                console.log('[ChatView] Plan approved');
                this.continueWithSystemMessage('[User has reviewed and approved the implementation plan. You may proceed with Phase 2: Implementation. Execute the plan as described.]');
            },
            onRequestChanges: (feedback) => {
                console.log('[ChatView] User requested changes:', feedback);
                // Process as a normal user message
                this.processUserMessage(feedback || 'Please make some changes to this plan', false);
            }
        });

        // Append card to chat thread
        if (this.currentTaskCard && this.currentTaskCard.element) {
            this.currentTaskCard.element.appendChild(card.getElement());
        } else {
            this.chatThread.appendChild(card.getElement());
        }
    ...
    /**
     * Check if plan review is needed and show the card
     * Called after plan files are executed
     */
    async checkAndShowPlanReview() {
        console.log('[ChatView] checkAndShowPlanReview called');

        const { ipcRenderer } = require('electron');
        const root = window.peakProjectRoot || window.currentProjectRoot;
        if (!root) {
            console.log('[ChatView] No project root, skipping plan review');
            return;
        }

        // Check if plan files exist in .peak folder
        const planPath = require('path').join(root, '.peak', 'implementation_plan.md');
        const todoPath = require('path').join(root, '.peak', 'TODO.md');

        const files = [];

        try {
            const planResult = await ipcRenderer.invoke('project:read-file', planPath);
            if (planResult && !planResult.error && planResult.content) {
                files.push({ path: '.peak/implementation_plan.md', content: planResult.content });
            }
        } catch (err) {
            console.log('[ChatView] Could not read .peak/implementation_plan.md:', err);
        }

        try {
            const todoResult = await ipcRenderer.invoke('project:read-file', todoPath);
            if (todoResult && !todoResult.error && todoResult.content) {
                files.push({ path: '.peak/TODO.md', content: todoResult.content });
            }
        } catch (err) {
            console.log('[ChatView] Could not read .peak/TODO.md:', err);
        }

        if (files.length === 0) {
            console.log('[ChatView] No plan files found in .peak folder');
            return;
        }

        // Check if the last AI message asks for plan approval
        const recentHistory = this.client.history.slice(-3); // Check last 3 messages
        const planReviewPatterns = [
            /does this plan look good/i,
            /does the plan look good/i,
            /approve.*plan/i,
            /review.*plan/i,
            /shall I proceed/i,
            /ready to proceed/i,
        ...
        const needsReview = recentHistory.some(msg =>
            msg.role === 'assistant' && planReviewPatterns.some(pattern => pattern.test(msg.content))
        );

        if (needsReview) {
            console.log('[ChatView] ✓ Plan review needed');

            // Check if auto-accept plan is enabled
            const autoAcceptPlan = localStorage.getItem('peak-auto-accept-plan') === 'true';

            if (autoAcceptPlan) {
                console.log('[ChatView] Auto-accepting plan files (setting enabled)');
                // Notify user that plans were created
                this.inputBar.updateStatus('ready', 'Plan files created and auto-approved');

                // Trigger Task tab refresh
                window.dispatchEvent(new CustomEvent('peak-plan-files-updated'));
                ...
                console.log('[ChatView] Showing plan review card for manual approval');
                // Trigger Task tab refresh
                window.dispatchEvent(new CustomEvent('peak-plan-files-updated'));
                await this.showPlanReviewCard(files);
            }
        } else {
            console.log('[ChatView] No plan review request detected in recent messages');
        }
    }
    ...
    async processUserMessage(prompt, isAuto = false) {
        ...
        try {
            ...
            // CRITICAL FIX: Always create a new card for user messages to ensure history is visible.
            // Only reuse the card for auto-generated steps (e.g. agent self-correction).
            if (this.currentTaskCard && !agentSwitched && isAuto) {
                ...
                this.currentTaskCard = new UnifiedTaskCard(prompt, agentId, commitHash);
    ...
    async handleAutoExecution(containerElement) {
        ...
        if (pendingButtons.length > 0) {
            ...
            // CHECK FOR PLAN FILES - auto-execute these always
            const planFileButtons = pendingButtons.filter(btn => {
                const path = btn.dataset.path || '';
                const fileName = path.split('/').pop(); // Get just the filename
                console.log('[ChatView] Checking button path:', path, 'fileName:', fileName);
                return fileName === 'implementation_plan.md' ||
                    fileName === 'TODO.md' ||
                    fileName === 'task.md' ||
                    fileName === 'walkthrough.md';
            });

            if (planFileButtons.length > 0) {
                console.log('[ChatView] ✓ Auto-executing plan files:', planFileButtons.length);
                for (const btn of planFileButtons) {
                    console.log('[ChatView] ✓ Auto-executing plan file:', btn.dataset.path);
                    await this.executeToolAction(btn);
                }

                // After plan files are created, check if we should show plan review card
                setTimeout(async () => {
                    await this.checkAndShowPlanReview();
                }, 500);

                return;
            } else {
                console.log('[ChatView] No plan files detected in pending buttons');
            }
            ...
            if (allAutoAccept) {
                ...
                if (this.isAutoMode || true) { // Force auto-continue for now as requested
                    setTimeout(() => {
                        this.continueWithSystemMessage('[Tools executed successfully. Continue processing.]');
                    }, 500);
                }
            ...
            this.inputBar.showReviewControls(
                ...
                async () => { // On Accept
                    ...
                    setTimeout(() => {
                        this.continueWithSystemMessage('[All pending actions accepted and executed. Continue processing.]');
                    }, 500);
                },
                () => { // On Reject
...
If REJECTED, explain why and provide a corrected version if possible.
    ...
    async checkAutoAccept() {
        ...
        if (buttonsToClick.length > 0) {
            ...
            this.continueWithSystemMessage('[Batch operations completed successfully. Continue processing.]');
        }
    ...
    async acceptAllPending() {
        ...
        const hasErrors = this.chatThread.querySelectorAll('button.error').length > 0;

        // Smart continuation: Always continue if there were errors so AI can see them and fix
        // Otherwise check if the AI's response indicates completion
        let shouldContinue = hasErrors; // Always continue on errors

        if (!hasErrors && this.currentTaskCard && this.currentTaskCard.element) {
        ...
        if (shouldContinue) {
            if (hasErrors) {
                console.log('[ChatView] Accept All complete - errors occurred, continuing so AI can see them');
                this.continueWithSystemMessage('[Tool execution completed with errors. Please review the error messages above and correct the issues.]');
            } else {
                console.log('[ChatView] Accept All complete - continuing to next phase');
                this.continueWithSystemMessage('[All changes accepted. Ready to proceed to next phase.]');
            }
        } else {
    ...
    async executeToolAction(btn, options = {}) {
        ...
        if (serverId) {
            ...
            try {
                ...
                // --- FIX: Resolve relative paths for filesystem tools ---
                // The filesystem server resolves relative paths against its CWD (app root),
                // so we must resolve them against the project root here.
                if (serverId === 'filesystem' && window.currentProjectRoot) {
                    const pathKeys = ['path', 'source', 'destination', 'directory'];
                    for (const key of pathKeys) {
                        if (args[key] && typeof args[key] === 'string') {
                            // Resolve if not absolute
                            if (!path.isAbsolute(args[key])) {
                                const original = args[key];
                                args[key] = path.resolve(window.currentProjectRoot, args[key]);
                                console.log(`[ChatView] Resolved relative path for ${key}: ${original} -> ${args[key]}`);
                            }
                        }
                    }
                    // Handle 'paths' array for read_multiple_files
                    if (args.paths && Array.isArray(args.paths)) {
                        args.paths = args.paths.map(p => {
                            if (typeof p === 'string' && !path.isAbsolute(p)) {
                                const resolved = path.resolve(window.currentProjectRoot, p);
                                console.log(`[ChatView] Resolved relative path in paths[]: ${p} -> ${resolved}`);
                                return resolved;
                            }
                            return p;
                        });
                ...
                // Auto-continue with tool result
                this.continueWithSystemMessage(`Tool Output (${toolName}):\n\`\`\`\n${outputText}\n\`\`\``);
        ...
        // FIX: Handle Generic Tool Rejection
        if (btn.classList.contains('tool-reject-btn')) {
            const container = btn.closest('.file-edit-line') || btn.closest('.tool-line'); // Added tool-line support
            if (container) {
                const runBtn = container.querySelector('.tool-action-btn') || container.querySelector('.tool-run-btn') || container.querySelector('.tool-delete-btn'); // Added other button types
                if (runBtn) {
                    const toolName = runBtn.dataset.tool || runBtn.dataset.type || 'unknown';
                    console.log(`[ChatView] User rejected tool: ${toolName}`);
                    ...
                    // Notify AI
                    this.appendMessage('system', `User rejected execution of tool: **${toolName}**`);
                    // Trigger AI response with user rejection context
                    this.continueWithSystemMessage(`User rejected execution of tool: **${toolName}**. Reason: Please propose an alternative or ask for clarification.`);
                }
            }
            return;
        }

        // FIX: Handle Run Command
        if (btn.classList.contains('tool-run-btn')) {
            const cmd = decodeURIComponent(btn.dataset.cmd);
            console.log(`[ChatView] Executing command: ${cmd}`);
            ...
            try {
                // Execute via IPC
                // Note: project:run-command returns { success, output, error }
                const result = await require('electron').ipcRenderer.invoke('project:run-command', cmd);
                ...
                // Show Output in Card if possible
                const card = btn.closest('.command-card');
                if (card) {
                    const outputDiv = card.querySelector('.file-code-collapsed');
                    if (outputDiv) {
                        outputDiv.style.display = 'block';
                        // Create or update output section
                        let outputContent = outputDiv.querySelector('.command-output-compact pre');
                        if (!outputContent) {
                            // Inject structure if missing (CommandCard might not have rendered it if output was null initially)
                            const newOutputStruct = document.createElement('div');
                            newOutputStruct.className = 'command-output-compact';
                            newOutputStruct.style.cssText = 'margin-top:8px; padding-top:8px; border-top:1px solid var(--border-color);';
                            newOutputStruct.innerHTML = `
                            ...
                            `;
                            outputDiv.appendChild(newOutputStruct);
                            outputContent = newOutputStruct.querySelector('pre');
                        }
                ...
                // Auto-continue with command result
                this.continueWithSystemMessage(`Command Executed: \`${cmd}\`\nOutput:\n\`\`\`\n${result.output || ''}\n\`\`\``);

            } catch (e) {
                console.error('[ChatView] Command failed:', e);
                btn.innerHTML = '<i data-lucide="alert-triangle"></i> Failed';
                btn.classList.add('error');
                this.appendMessage('system', `❌ Command Failed: ${e.message}`);

                // Add error to history so AI knows
                this.client.history.push({
                    role: 'system',
        ...
        // FIX: Handle Delete File
        if (btn.classList.contains('tool-delete-btn')) {
            const pathToDelete = decodeURIComponent(btn.dataset.path);
            console.log(`[ChatView] Deleting file: ${pathToDelete}`);
            ...
            try {
                ...
                const fullPath = require('path').join(root, pathToDelete);

                // Use project:delete-file IPC
                const result = await require('electron').ipcRenderer.invoke('project:delete-file', fullPath);
                ...
                // Auto-continue with deletion notification
                this.continueWithSystemMessage(`File deleted: ${pathToDelete}`);

            } catch (e) {
                console.error('[ChatView] Delete failed:', e);
                btn.innerHTML = '<i data-lucide="alert-triangle"></i> Failed';
                btn.classList.add('error');
                this.appendMessage('system', `❌ Delete Failed: ${e.message}`);
            }
            return;
        }
        ...
        if (btn.classList.contains('tool-create-btn') || btn.classList.contains('file-action-btn-compact')) {
            ...
            try {
                ...
                let writeResult;
                // Check for both 'modify' and 'update' since CodeThinkingCard sets edit_file to 'update' for auto-accept logic
                const toolName = btn.closest('[data-tool-name]')?.dataset?.toolName;
                const hasMarkers = content.includes('<<<<<<< SEARCH');
                const isExplicitEditTool = toolName === 'edit_file' || toolName === 'modify_file';

                // Only use edit-file logic if it's explicitly an edit tool OR if content contains markers
                // This handles 'update_file' (overwrite) correctly by falling back to write-file
                if ((type === 'modify' || type === 'update') && (isExplicitEditTool || hasMarkers)) {
                    console.log(`[ChatView] Editing file via IPC (search/replace): ${fullPath}`);
                    writeResult = await require('electron').ipcRenderer.invoke('project:edit-file', fullPath, content);
                } else {
                    console.log(`[ChatView] Overwriting file via IPC: ${fullPath}`);
                    writeResult = await require('electron').ipcRenderer.invoke('project:write-file', fullPath, content);
                }
                ...
                console.error('[ChatView] File write failed details:', JSON.stringify(err, Object.getOwnPropertyNames(err)));
                // Enhanced error message with suggestions
                let errorMsg = `Error writing file: ${err.message || 'Unknown error'}`;

                if (err.message.includes('File not found') || err.message.includes('ENOENT')) {
                    const ErrorMessageHelper = require('../../../services/ErrorMessageHelper');
                    const root = window.peakProjectRoot || '';
                    errorMsg = ErrorMessageHelper.fileNotFoundMessage(root, path, type === 'modify' ? 'Edit' : 'Write');
                }
        ...
        if (btn.classList.contains('tool-view-btn')) {
            ...
            try {
                ...
                if (!options.skipAutoContinue) {
                    this.continueWithSystemMessage(`File Content (${path}):\n\`\`\`\n${content}\n\`\`\``);
                }
    ...
    async handleToolAction(e) {
        ...
        if (acceptAllBtn) {
            ...
            if (AgentOrchestrator.isLoopActive) {
                ...
                this.continueWithSystemMessage('[All actions completed successfully. Continue.]');
            }
            return;
        }
    ...
    async captureLiveViewAndSendToAI(triggerButton = null, options = {}) {
        try {
            ...
            // Trigger Auto-Continue with the snapshot content
            if (!options.skipAutoContinue) {
                this.continueWithSystemMessage(messageContent);
            }
    ...
    async readUrlAndSendToAI(url, triggerButton = null, options = {}) {
        try {
            ...
            // Trigger Auto-Continue with URL content
            if (!options.skipAutoContinue) {
                this.continueWithSystemMessage(messageContent);
            }
    ...
    async getProblemsAndSendToAI(triggerButton = null, options = {}) {
        try {
            if (window.peakGetDiagnostics) {
                ...
                // Trigger Auto-Continue with problems list
                if (!options.skipAutoContinue) {
                    this.continueWithSystemMessage(fullMessage);
                }
    ...
    async sendFileContentToAI(relPath, triggerButton = null, options = {}) {
        ...
        try {
            ...
            if (!options.skipAutoContinue) {
                this.continueWithSystemMessage(message);
            }
    ...
    async listDirectoryAndSendToAI(relPath, recursive, targetCard = null, options = {}) {
        ...
        try {
            ...
            // Trigger Auto-Continue with directory listing
            if (!options.skipAutoContinue) {
                this.continueWithSystemMessage(messageContent);
            }
    ...
    async runSearchAndSendToAI(query, options = {}) {
        try {
            ...
            if (!options.skipAutoContinue) {
                this.continueWithSystemMessage(outputContent);
            }
